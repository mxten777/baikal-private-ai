    ...
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-in">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <div>
            <h1 className="text-xl sm:text-[28px] font-extrabold text-gray-100 tracking-tight">사용자 관리</h1>
            <p className="text-sm text-gray-500 mt-1 font-medium">시스템 사용자 계정 관리</p>
          </div>
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${
              showCreate
                ...
                : 'bg-baikal-600 text-white hover:bg-baikal-500 shadow-lg shadow-baikal-600/20'
            }`}
          >
        ...
        {/* 생성 폼 */}
        {showCreate && (
          <div className="bg-white/[0.03] rounded-xl border border-white/[0.06] p-6 mb-6 animate-fade-in-up">
            <h3 className="text-[15px] font-bold text-gray-200 mb-4 flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-baikal-500/15 flex items-center justify-center">
                <HiOutlinePlus className="w-3.5 h-3.5 text-baikal-400" />
              </div>
              새 사용자
            </h3>
            <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-3 sm:items-end">
              <div className="flex-1">
                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em] mb-1.5">아이디</label>
                <input type="text" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} className="w-full px-3.5 py-2.5 bg-white/[0.03] border border-white/[0.06] rounded-lg text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-baikal-500/40 focus:ring-1 focus:ring-baikal-500/20 transition-all" placeholder="사용자 아이디" />
              </div>
              <div className="flex-1">
              ...
              <button type="submit" className="px-6 py-2.5 rounded-lg bg-baikal-600 text-white text-sm font-semibold hover:bg-baikal-500 shadow-lg shadow-baikal-600/20 transition-all w-full sm:w-auto">생성</button>
            </form>
          </div>
        )}
        ...
        <div className="bg-white/[0.03] rounded-xl border border-white/[0.06] overflow-hidden">
          <div className="px-6 py-4 border-b border-white/[0.04]">
            <div className="flex items-center gap-3">
              <h2 className="text-[15px] font-bold text-gray-200">사용자 목록</h2>
              <span className="px-2 py-0.5 rounded-lg bg-white/[0.06] text-[11px] font-bold text-gray-400">{users.length}</span>
            </div>
          </div>
            ...
            <table className="w-full min-w-[500px]">
              <thead>
                <tr className="border-b border-white/[0.04]">
                  <th className="px-4 sm:px-6 py-3 text-left text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">사용자</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">권한</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em]">상태</th>
                  <th className="px-4 sm:px-6 py-3 text-left text-[10px] font-bold text-gray-500 uppercase tracking-[0.1em] hidden sm:table-cell">생성일</th>
                ...
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 sm:px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-baikal-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0">
                          {user.username?.slice(0, 2).toUpperCase()}
                        </div>
                        <span className="text-[13px] font-semibold text-gray-300 truncate max-w-[80px] sm:max-w-none">{user.username}</span>
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-4">
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <button onClick={() => handleToggleActive(user)} className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-all ${
                        user.is_active ? 'bg-emerald-500/15 text-emerald-400 hover:bg-emerald-500/25' : 'bg-red-500/15 text-red-400 hover:bg-red-500/25'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${user.is_active ? 'bg-emerald-400' : 'bg-red-400'}`} />
                        {user.is_active ? '활성' : '비활성'}
                      </button>
                    </td>
                    <td className="px-4 sm:px-6 py-4 text-[13px] text-gray-500 hidden sm:table-cell">{new Date(user.created_at).toLocaleDateString('ko-KR')}</td>
                    <td className="px-4 sm:px-6 py-4">
                      <button onClick={() => handleDelete(user)} className="p-2 text-gray-600 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all" title="삭제">
