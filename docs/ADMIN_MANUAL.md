...
11. [KPI 대시보드](#11-kpi-대시보드)
12. [백업 및 복구](#12-백업-및-복구)
13. [폐쁼망 설치](#13-폐쁼망-설치)
14. [문제 해결 가이드](#14-문제-해결-가이드)
15. [보안 설정 가이드](#15-보안-설정-가이드)
16. [FAQ](#16-faq)
...
- 좌측 메뉴 → **관리자** → **"KPI 대시보드"** (또는 "시스템 설정")
- 관리자 권한(role: admin)이 있는 계정만 접근 가능합니다
- RAG 품질·성능·운영 지표를 5개 탭으로 구분하여 실시간 확인합니다
- 우측 상단 **"새로고침"** 버튼으로 최신 데이터를 불러올 수 있습니다
...
| 지표 | 설명 | 목표값 |
|------|------|--------|
| **총 질의 수** | 누적 AI 질의응답 건수 | — |
| **평균 신뢰도** | 답변 신뢰도 점수 평균 (0~100%) | — |
| **평균 응답시간** | 질문→답변 완료까지 평균 시간(ms) | ≤ 8,000ms |
| **활성 사용자 (7일)** | 최근 7일 로그인+질의 사용자 수 및 비율 | ≥ 60% |
| **Query Success Rate** | 사용자 👍 피드백 비율 | ≥ 70% |
| **Source Click-through Rate** | 출처 원문 클릭 비율 | ≥ 30% |

화면 하단에 **최근 7일 주간 질의 추이 막대 차트**가 표시됩니다.

#### ② Retrieval 탭

RAG 검색 파이프라인의 단계별 응답 지연시간을 측정합니다.

| 지표 | 설명 | 목표값 |
|------|------|--------|
| **검색 단계** (`retrieval_ms`) | 벡터+BM25 하이브리드 검색 소요시간 | ≤ 1,500ms |
| **Reranking** (`reranking_ms`) | Cross-encoder 재정렬 소요시간 | ≤ 500ms |
| **LLM 생성** (`llm_ms`) | LLM 답변 생성 소요시간 | — |

수평 스택 바로 각 단계의 시간 비중이 시각화됩니다. 병목 구간을 즉시 파악할 수 있습니다.

#### ③ Trust 탭 (Answer Trust)

답변의 신뢰도 분포와 사용자 수용률을 확인합니다.

| 지표 | 설명 |
|------|------|
| **High (≥70%)** | 충분한 근거가 있는 고신뢰도 답변 수 |
| **Medium (40~70%)** | 중간 신뢰도 답변 수 |
| **Low (<40%)** | 근거 부족 저신뢰도 답변 수 |
| **Answer Acceptance Rate** | 사용자 긍정 피드백 비율 (목표 ≥70%) |
| **출처 클릭률** | 근거 원문 확인 비율 (목표 ≥30%) |

> **⚠ 저신뢰도 알림**: Low 건수가 발생하면 빨간색 경고 박스가 표시됩니다. 관련 문서 추가 업로드를 권장합니다.

#### ④ Operations 탭

Ollama에 설치된 LLM 모델 목록을 확인하고 현재 활성 모델을 전환합니다.

- **모델 목록**: 설치된 모델명과 파일 크기 표시
- **활성 모델 전환**: 모델 행 클릭 → 즉시 전환 (재시작 불필요)
- **임베딩 모델**: 현재 사용 중인 임베딩 모델 표시 (변경 불가 — 변경 시 전체 문서 재처리 필요)

> **주의**: 임베딩 모델(`bge-m3`)은 이 화면에서 변경할 수 없습니다. 변경이 필요한 경우 `.env`를 수정 후 모든 문서를 재업로드해야 합니다.

#### ⑤ Governance 탭

보안 정책 준수 현황과 전체 질의 감사 로그를 확인합니다.

| 지표 | 설명 | 목표값 |
|------|------|--------|
| **정책 위반 전체** | Guardrail에 의해 차단된 누적 질문 수 | — |
| **오늘 위반** | 당일 차단 건수 | 0건 |
| **보안 상태** | 안전 / 양호 / 경고 3단계 | 안전 |

- **최근 정책 위반 내역**: 차단된 질문 내용·차단 사유·시각이 목록으로 표시됩니다
- **질의 감사 로그**: 전체 질의 내역, 사용자, 신뢰도, 응답시간, 피드백, 출처 클릭 여부 확인 가능

### 11.3 KPI 목표값 요약

| 분류 | KPI | 목표값 |
|------|-----|--------|
| 품질 | Query Success Rate | **≥ 70%** |
| 품질 | 평균 신뢰도 | **≥ 70%** |
| 성능 | 총 응답시간 | **≤ 8,000ms** |
| 성능 | 검색 단계 | **≤ 1,500ms** |
| 성능 | Reranking 단계 | **≤ 500ms** |
| 활용 | 활성 사용자율 (WAU) | **≥ 60%** |
| 활용 | Source Click-through Rate | **≥ 30%** |
| 보안 | 일일 정책 위반 | **0건** |
...
> **문서 끝** | BAIKAL Private AI 관리자 매뉴얼 v1.3
